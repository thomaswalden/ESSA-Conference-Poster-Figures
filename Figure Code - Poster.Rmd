...
label.testsRPD_1 <- data.frame(session_f = c("FWBFR"),
 ...
 RPDPLOT <- ggplot(RPDmeans,aes(x=session_f, y=RPD))+
  ...
  theme(axis.title.x=element_text(size=18),
        ...
        strip.text.x=element_text(size=14,face="italic"),
        legend.title = element_blank())+
        geom_text(data = label.testsRPD_1, label = " b - c", position = pd)+
        geom_text(data = label.testsRPD_2, label = " b", position = pd)+
...
```{r}
ggsave(plot=SUBFIG,"C:\\Users\\thoma\\OneDrive - Murdoch University\\Desktop\\PhD 2020\\Conferences\\ESSA\\WALK STUDY\\SUBFIG(ESSA Poster).png",units = "in",width = 15.0,height = 11.0)
```
